...
export default function BoardKanban({ partidos, arbitros, onActualizar }) {
  ...
  const asignados = partidos.filter(p => p.estado === "asignado")
  ...
  const seleccionarArbitro = (arbitro) => {
  ...
  const confirmarAsignacion = async () => {
    ...
    try {
      ...
      await fetch("http://127.0.0.1:8000/asignaciones", {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          partido_id: partidoSel.id,
          arbitro_id: arbitroSel.id,
          rol
        })
      })
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
          ...
          {arbitros.length === 0 && (
            <p className="text-sm text-gray-400">No hay árbitros disponibles</p>
          )}
          {arbitros.map(a => (
            <div
              key={a.id}
              onClick={() => seleccionarArbitro(a)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={confirmarAsignacion} disabled={cargando}
