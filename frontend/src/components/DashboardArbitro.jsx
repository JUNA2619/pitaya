...
export default function DashboardArbitro({ usuario, onLogout }) {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      const [resAsig, resDisp, resNoti] = await Promise.all([
        fetch(`${API}/arbitros/mis-asignaciones`, { headers }),
        fetch(`${API}/arbitros/disponibilidad`, { headers }),
        fetch(`${API}/arbitros/notificaciones`, { headers })
      ])
  ...
  const responderAsignacion = async (id, estado) => {
    ...
    try {
      ...
      cargarDatos()
    } catch { console.error("Error respondiendo") } finally { setProcesando(null) }
  }
  ...
  const guardarDisponibilidad = async () => {
    ...
    try {
      const token = localStorage.getItem("token")
      await fetch(`${API}/arbitros/disponibilidad`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      ...
      cargarDatos()
    } catch { console.error("Error guardando") } finally { setGuardando(false) }
  }

  const eliminarDisponibilidad = async (id) => {
    const token = localStorage.getItem("token")
    await fetch(`${API}/arbitros/disponibilidad/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } })
    cargarDatos()
  }

  const marcarLeida = async (id) => {
    const token = localStorage.getItem("token")
    await fetch(`${API}/arbitros/notificaciones/${id}/leer`, { method: "PATCH", headers: { Authorization: `Bearer ${token}` } })
    cargarDatos()
  }
          ...
          {["agenda", "disponibilidad", "notificaciones", "reporte"].map(v => (
            <button key={v} onClick={() => setVista(v)}
              ...
              {v === "notificaciones" && noLeidas > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-4 h-4 rounded-full flex items-center justify-center">{noLeidas}</span>
              )}
            </button>
          ))}
                ...
                {pendientes.map(a => (
                  <div key={a.id} className="bg-white border border-yellow-200 rounded-xl p-4 mb-3">
                    ...
                    <div className="flex gap-2 mt-3">
                      <button onClick={() => responderAsignacion(a.id, "rechazado")} disabled={procesando === a.id}
                        className="flex-1 text-xs border border-red-200 text-red-600 py-1.5 rounded-lg hover:bg-red-50 disabled:opacity-50">Rechazar</button>
                      <button onClick={() => responderAsignacion(a.id, "confirmado")} disabled={procesando === a.id}
                        className="flex-1 text-xs bg-purple-600 text-white py-1.5 rounded-lg hover:bg-purple-700 disabled:opacity-50">Confirmar</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            <div className="bg-white border border-gray-200 rounded-xl p-4 mb-6">
              ...
              <div className="grid grid-cols-3 gap-3 mb-3">
                <div>
                  ...
                  <input type="date" value={nuevaDisp.fecha} onChange={e => setNuevaDisp({ ...nuevaDisp, fecha: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Desde</label>
                  <input type="time" value={nuevaDisp.hora_inicio} onChange={e => setNuevaDisp({ ...nuevaDisp, hora_inicio: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-400" />
                </div>
                <div>
                  <label className="text-xs text-gray-500 mb-1 block">Hasta</label>
                  <input type="time" value={nuevaDisp.hora_fin} onChange={e => setNuevaDisp({ ...nuevaDisp, hora_fin: e.target.value })}
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-purple-400" />
                </div>
              </div>
            ...
            {disponibilidad.sort((a, b) => a.fecha.localeCompare(b.fecha)).map(d => (
              <div key={d.id} className="bg-white border border-gray-200 rounded-xl p-4 mb-3 flex justify-between items-center">
                ...
                <button onClick={() => eliminarDisponibilidad(d.id)} className="text-xs text-red-500 hover:text-red-700">Eliminar</button>
              </div>
            ))}
          </div>
        )}
          ...
          <div>
            ...
            {notificaciones.map(n => (
              <div key={n.id} className={`bg-white border rounded-xl p-4 mb-3 flex justify-between items-start ${!n.leida ? "border-purple-200" : "border-gray-200"}`}>
                ...
                {!n.leida && <button onClick={() => marcarLeida(n.id)} className="text-xs text-purple-600 hover:underline ml-4 flex-shrink-0">Marcar leída</button>}
              </div>
            ))}
          </div>
        )}
        {!cargando && vista === "reporte" && <ReporteMensual token={localStorage.getItem("token")} />}
      </div>
    </div>
  )
}